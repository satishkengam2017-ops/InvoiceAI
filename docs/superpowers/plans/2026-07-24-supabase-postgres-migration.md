...
- **Corrected during Task 3** (verified live against Supabase, commit `c18467f`): `DATABASE_URL` and `DATABASE_URL_DIRECT` both use Supabase's **direct** connection (port 5432) — not the transaction-mode pooler (port 6543) originally specified for `DATABASE_URL`. The pooler is incompatible with SQLAlchemy's asyncpg dialect: its automatic JSON codec setup runs on every new connection using asyncpg's own internal, deterministic prepared-statement naming, which collides across pooled connections (`DuplicatePreparedStatementError`). This isn't fixable via SQLAlchemy's documented PgBouncer workaround (`NullPool` + `prepared_statement_name_func`), since the codec setup bypasses that override path entirely. The pooler exists for many-short-lived-connection (serverless) use cases; InvoiceAI's backend is a single persistent server, exactly what Supabase's own docs recommend the direct connection for instead.
